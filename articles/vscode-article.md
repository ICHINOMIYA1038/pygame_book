...
![ドラえもんカラーテーマ](/images/vscode-doraemon-theme.png)

何のアニメかはぜひ、ご想像ください。

作ったきっかけは、claude code で何か面白いことできないかなと思い、setting.json の色を変えてもらえれば、自作テーマも余裕のでは？と思って作りました。(設定ファイルをいじらせることになるので、真似する際には要注意)
...
そこで、もっと見やすいカラーテーマを作るために VSCode のカラーテーマやしンタックスハイライトの色の仕組みについて詳しく調べてみて、さらに実際の独自のカラーテーマを作成してみました。
...
まず、VS コードのカラーテーマを作るための資料です。
https://code.visualstudio.com/api/extension-guides/color-theme?ref=trap.jp#create-a-new-color-theme

この記事によると、一番簡単な方法は既存のカラーテーマをカスタマイズすることです。

vscode の設定画面から workbench.colorCustomization を検索
...
このようにテーマカラーに関しては、setting.json を変更すると変更できることがわかります。
...
次は、シンタックスハイライトの色を変えます。

シンタックスハイライトの色については二つの方法があります。

- .tmTheme を使ってテーマ自体を変える方法
- editor.tokenColorCustomizations を使って テーマを上から部分的に変える方法
...
しンタックスハイライトは、特定のキーワードに色をつける仕組みです。
...
VSCODE はトークン化のエンジンとして、TextMate 文法を使用しています。
TextMate 文法では、正規表現を用いてコード解析してトークンを識別しています。

[参考]TextMate の文法の仕組みは以下のドキュメントが参考になりました。
...
それは言語ごとに`.tmLanguage.json`が用意されているからです。

拡張機能でシンタックスハイライトを入れている方も多いかと思いますが、
そのシンタックスハイライトプラグインは、TextMate 文法ファイル（.tmLanguage.json など）を提供しています。

`.tmLanguage.json` に関しては、
https://techblog.kayac.com/vscode-extension-syntax-highlight　がとても参考になりました。

特定の言語にどの拡張機能がハイライトしているかを確認するには以下の方法が良いかとおもいます。
正確な方法ではないかもしれませんが、私はこうしました。
...
[参考]拡張機能のファイルのありか
VSCode で mac の私の場合は以下の位置に拡張機能のディレクトリがありました。
"/Applications/Visual Studio
Code.app/Contents/Resources/app/extensions/"の中にあります。
...
現在どの文字がどのトークンに設定されているかを確認するには、以下の方法がおそらく一番早いです。

1. Cmd + Shift + P でコマンドパレットを開く
2. 「Developer: Inspect Editor Tokens and Scopes」 を実行
3. カーソル位置のトークンに適用されているスコープ（tmLanguage の定義）が表示される
...
| **`.tmTheme` `-color-theme.json` ** | **配色定義（Theme）**   | 文法で分類されたトークンに色を割り当てる     |

## テーマファイルの作成

今回は `yo code`（Yeoman + generator-code）を使ってカラーテーマを作ります。
yo code は vscode の拡張機能やカラーテーマを簡単に作ることができます。
...
実際に色の変化を確認するため、以下の手順で開発画面を開きます。

1. **F5** を押す（デバッグ実行）
2. 新しい VS Code ウィンドウが開く
3. `Cmd + K, Cmd + T`（テーマ選択）
4. 「My Test Theme(自分でつけた名前)」を選択
5. テーマファイルに編集を入れた場合、command + shift + R でリロード

例えば、上述の python の複数行のコメントの中の色を変える場合には以下のように書きます。

````
    {
      "name": "長いコメントをピンク色にしてみるs",
...
このようにカラーテーマの仕組みを理解することで自由自在に Visual Studio コードの色を自由自在に変えることができます。

## おまけ

好きな文字を好きな色に変えることができるということは、アスキーアートに好きな色をつけることができるんじゃないかと閃いたので、作ってみました。

1. まず、絵を描きます。猫にしました。

![手描きの猫](/images/cat-drawing.jpg)

2. gemini の nanobanana を使って色をつけます。
...
いかがだったでしょうか。今回の内容は AI とは直接関係がありませんが、AI を使いこなすためには、遊び心が大事だと思っています。
今回のような遊びは AI があるからこそできる研究でした。
このような楽しい技術研究をしては Tips 会で共有することができる弊社は AI をたくさん使いたい！というエンジニアにはおすすめです。
